...
class MotorController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('kilometer')) {
            $request->merge(['kilometer' => preg_replace('/[^0-9]/', '', $request->kilometer)]);
        }
        ...
        $request->validate([
            ...
            'detail_spesifikasi' => 'nullable|string|max:1000', // Ubah jadi nullable
        ]);
        ...
        if ($request->has('detail_spesifikasi')) {
            // Jika asalnya dari form Detail Spesifikasi, putar balik ke halaman spesifikasi lagi
            return redirect()->back()->with('success', 'Detail spesifikasi berhasil diperbarui!');
        } else {
            // Jika asalnya dari form Edit Kendaraan biasa, kembalikan ke tabel Data Kendaraan
